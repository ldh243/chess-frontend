<template>
  <div class="trainingTemplate">
    <v-navigation-drawer fixed v-model="drawer" app>
      ...
      <v-list>
        ...
        >
          ...
          <v-list-tile
            v-for="subItem in item.lessons"
            v-bind:key="subItem.lessonTitle"
            class="removeListPadding"
            @click="loadContent(subItem.lessonID)"
          >
            <v-list-tile-action>
              <v-icon v-if="subItem.active" class="blue--text">play_circle_outline</v-icon>
              <v-icon v-else-if="subItem.lessonComplete" class="green--text">done</v-icon>
              <v-icon v-else>radio_button_unchecked</v-icon>
            </v-list-tile-action>
            <v-list-tile-content>
              <v-list-tile-title
                v-bind:class="{'lessonComplete': subItem.lessonComplete}"
    ...
    <v-toolbar color="indigo" dark fixed app class="elevation-0">
      ...
      <v-toolbar-title>Học Với Video</v-toolbar-title>
    </v-toolbar>
      ...
      courseContentList: [
        {
          ...
          lessons: [
            {
              lessonID: 'lesson-1',
              lessonTitle: 'Bài giảng 1: Giới thiệu quân cờ ',
            ...
            {
              lessonID: 'lesson-2',
              lessonTitle: 'Bài giảng 2: Giới thiệu bàn cờ',
              lessonComplete: false,
              active: false,
              nextLesson: 'lesson-3'
            },
            {
              lessonID: 'lesson-3',
              lessonTitle: 'Bài giảng 3: Kỹ năng cần thiết',
              lessonComplete: false,
              active: false,
              nextLesson: 'lesson-4'
            },
            {
              lessonID: 'lesson-4',
              lessonTitle: 'Bài giảng 4: Cách nhập thành',
              lessonComplete: false,
              active: false,
              nextLesson: 'lesson-5'
            },
            {
              lessonID: 'lesson-5',
              lessonTitle: 'Bài giảng 5: Nước đi hiệu quả',
              lessonComplete: false,
              active: false,
              nextLesson: 'lesson-6'
            },
            {
              lessonID: 'lesson-6',
              lessonTitle: 'Bài giảng 6: Phá thế',
              lessonComplete: false,
              active: false,
              nextLesson: 'lesson-7'
            },
            {
              lessonID: 'lesson-7',
              lessonTitle: 'Bài giảng 7: Chiếu tướng',
              lessonComplete: false,
              active: false,
              nextLesson: 'lesson-7'
            }
      ...
      lessonContent: [
        {
          ...
          lessonTitle: 'Bài giảng 1: Giới thiệu quân cờ ',
          lessonDescription:
            'A short paragraph on what the person is going to learn in this lesson.',
  ...
  methods: {
    loadContent: function(lessonID) {
      ...
      for (let index = 0; index < this.courseContentList.length; index++) {
        ...
        for (
          let subIndex = 0;
          subIndex < this.courseContentList[index].lessons.length;
          subIndex++
        ) {
          if (
            lessonID ===
            this.courseContentList[index].lessons[subIndex].lessonID
          ) {
      ...
      for (let index = 0; index < this.courseContentList.length; index++) {
        for (
          ...
          if (
            ...
            this.courseContentList[index].lessons[
              subIndex
            ].lessonComplete = true
            this.loadContent(
              this.courseContentList[index].lessons[subIndex].nextLesson
            )
            this.calculateCompletedPercentage()
            break
          }
    ...
    calculateCompletedPercentage: function() {
      ...
      for (let index = 0; index < this.courseContentList.length; index++) {
        for (
          ...
          if (
            this.courseContentList[index].lessons[subIndex].lessonComplete ===
            true
          ) {
            completedLessons++
          }
    ...
    audioLoaded: function() {
      ...
      if (this.audioPlayer !== null) {
        ...
        if (!isNaN(this.audioPlayer.duration)) {
          ...
          this.trackLength = this.formatTrackTime(
            Math.floor(this.audioPlayer.duration)
          )
    ...
    onAudioTimeUpdate: function() {
      ...
      this.elapsedTime = this.formatTrackTime(this.audioPlayer.currentTime)
      this.remainingTime = this.formatTrackTime(
